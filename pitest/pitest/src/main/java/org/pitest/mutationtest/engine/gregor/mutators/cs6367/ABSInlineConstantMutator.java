 ...
 */
public class ABSInlineConstantMutator implements MethodMutatorFactory {
  ...
  private final class InlineConstantVisitor extends MethodVisitor {
    ...
    private void mutate(final Integer constant) {
      ...
      case 1:
        replacement = constant*(-1);//Integer.valueOf(0);
        break;
      case Byte.MAX_VALUE:
        replacement = constant*(-1);//Integer.valueOf(Byte.MIN_VALUE);
        break;
      case Short.MAX_VALUE:
        replacement = constant*(-1);//Integer.valueOf(Short.MIN_VALUE);
        break;
      default:
        replacement = constant*(-1);//Integer.valueOf(constant + 1);
        break;
      }
    ...
    private <T extends Number> boolean shouldMutate(final T constant,
      ...
      final MutationIdentifier mutationId = this.context.registerMutation(
          ABSInlineConstantMutator.this, "Substituted " + constant + " with "
              + replacement);
